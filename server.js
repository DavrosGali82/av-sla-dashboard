...
function computeMetrics(cases, hcvRows, officeHCVSummary=[], reportingMonthKey=null) {
  ...
  return {
    ...
    resolutionBreakdown:{
      ...
      siteVisitWarranty:cases.filter(c=>c.warranty&&c.siteVisit).length,
      remote:           cases.filter(c=>c.remoteResolution).length,
      remoteSLA:        cases.filter(c=>!c.warranty&&c.remoteResolution).length,
      remoteWarranty:   cases.filter(c=>c.warranty&&c.remoteResolution).length,
      unrecorded:       cases.filter(c=>!c.siteVisit&&!c.remoteResolution&&c.status==="Closed").length,
    },
